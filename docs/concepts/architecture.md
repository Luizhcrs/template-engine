...
└── section_mapper/
    ...
    ├── source_profiler.py   profile_source (LLM-driven mapper: polymorphic input, body fallback)
    ...
    ├── template_renderer.py render_pages (LLM-driven mapper: docx → PDF → PNG for vision)
    ├── plan_cache.py        load_plan / save_plan (LLM-driven mapper: sha256-keyed plan cache)
